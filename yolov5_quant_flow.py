# YOLOv5 Ì†ΩÌ∫Ä by Ultralytics, GPL-3.0 license
"""
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    ### QAT Start
    if opt.qat:
        from qat_utils import calibrate_model, evaluate_accuracy, build_sensitivity_profile
        ...
        if opt.num_calib_batch > 0 and (not opt.skip_eval_accuracy):
            map50_calibrated, map_calibrated = evaluate_accuracy(model, opt, val_loader, data_dict)        
        ...
        if opt.sensitivity:
            build_sensitivity_profile(model, opt, val_loader, data_dict)
    ### QAT End
        ...
        if RANK in {-1, 0}:
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                if best_fitness == fi:
                    ...
                    ### QAT Start
                    if opt.qat:
                        from qat_utils import export_onnx_qat
                        # Export to ONNX
                        import pdb; pdb.set_trace()
                        export_onnx_qat(deepcopy(de_parallel(model)), str(w) + '/best.onnx')
                    ### QAT End
...
def parse_opt(known=False):
    ...
    ### QAT Start
    # QAT arguments
    parser.add_argument('--qat', action='store_true', help='Enable QAT')
    parser.add_argument('--model-name', default='yolov5m', help='model name: default yolov5m')
    parser.add_argument('--num-calib-batch', default=32, type=int,
                        help='Number of batches for calibration. 0 will disable calibration. (default: 32)')
